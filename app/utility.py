...
# modify for new server in 2024 ... works better to import the entire file
# import discrepancy
# from fr3d.geometry import discrepancy
from discrepancy import matrix_discrepancy, relative_discrepancy
# from fr3d.geometry.discrepancy import matrix_discrepancy, relative_discrepancy
...
def get_nt_position_index(data):
    ...
    for ife, nt_positions in data.items():
...
def get_name_count(data):
    ...
    count_values = [(k,v) for k,v in count_dict.items() if k is not None]
...
def get_correspondence_across_species(param_dict):
    base_url = "https://rna.bgsu.edu/correspondence/map_across_chains?id="
    suffix_url = "&format=json"
    complete_url = base_url + str(",".join(param_dict['selection'])) +  "&scope=" + str(param_dict['scope']) + "&depth=" + str(param_dict['depth']) + "&resolution=" + str(param_dict['resolution']) + "A" + suffix_url
...
def get_sequence_variability(query_units_string):
    ...
    # revise for Python 2/3 compatibility
    count_dict = [(str(k),int(v)) for k,v in count_dict.items() if "-" not in k]
...
def format_correspondence_display(corr):
	...
	display_str = ""
    # revise for Python 2/3 compatibility
	for ife, correspondence in corr.items():
...
def check_missing_correspondence(data, query_length):
    ...
    # revise for Python 2/3 compatibility
    entries_with_missing_data = [k for k, v in data.items() if len(v) != query_length]
...
def check_missing_correspondence_new(corr_complete, corr_std, chain):
    ...
    # revise for Python 2/3 compatibility
    correspondence_lengths = [len(v) for k,v in corr_std.items()]
    ...
    # revise for Python 2/3 compatibility
    for ife, correspondence in corr_std.items():
...
def format_pairwise_interactions_display(corr_data, nt_pairs_positions, pairwise_data, query_len):
    ...
    # revise for Python 2/3 compatibility
    for ife, interaction_data in interactions_dict.items():
...
def format_pairwise_interactions(data):
    ...
    # Prebuild the interactions dict with empty strings
    interactions_dict = {
        ife: {nt_pair: "" for nt_pair in nt_pairs} for ife in ifes_list
    }

    # Populate the dictionary
    for ife, nt_pair_data in pairwise_data.items():
        for nt_pair in nt_pairs:
            if nt_pair in nt_pair_data:
                interactions_dict[ife][nt_pair] = nt_pair_data[nt_pair]
...
def build_loop_name(corr_complete):
    ...
    loop_names = []
    # revise for Python 2/3 compatibility
    for ife, loop_range in corr_complete.items():
...
def get_correspondence_positions(correspondence):
    ...
    positions_ref = {}
    # revise for Python 2/3 compatibility
    for ife, corr in correspondence.items():
        positions = {}
...
def process_query_units(query_units):
    ...
    ife = pdb + "|1|" + chain    # use model 1 since IFEs only use model 1 ... not true! see 7T1N
    ife = "|".join((pdb, model, chain))

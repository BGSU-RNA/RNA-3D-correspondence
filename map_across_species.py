...
http://rna.bgsu.edu/correspondence/map_across_species?id=5J7L|1|DA|C|2652:5J7L|1|DA|G|2668 covers a range

http://rna.bgsu.edu/correspondence/map_across_species?id=5J7L|1|AA|U|70,5J7L|1|AA|A|98&scope=Rfam&resolution=2.5A shows how to ask for a comma-separated list of unit ids.  It's small enough that you can actually understand the text output.
http://rna.bgsu.edu/correspondence/map_across_species?id=5J7L|1|DA|G|2655,5J7L|1|DA|U|2656,5J7L|1|DA|A|2665&scope=molecule maps the base triple in the sarcin-ricin loop across archaeal, bacterial, and eukaryotic LSU structures at 3.0A.  To make it easier to see, I attached the output in an Excel file.  One could check the Infernal alignments this way.  So far, they look good.
http://rna.bgsu.edu/correspondence/map_across_species?id=IL_4V9F_027&scope=Rfam&resolution=2.5A only does archaeal LSU structures
http://rna.bgsu.edu/correspondence/map_across_species?id=HL_4V9F_028 (default 3.0A, default scope = Rfam)
http://rna.bgsu.edu/correspondence/map_across_species?id=HL_4V9F_028&scope=molecule aligns across all three LSU Rfam families.  The alignment might not be all that good!
http://rna.bgsu.edu/correspondence/map_across_species?id=IL_6ZMI_019&format=text to specifically ask for text format.  This IL has one strand from 5.8S and one strand from the long LSU chain.  So the output has two sets of Rfam/EC/chain columns, to explain where each chain is coming from.  I copied and pasted the output into a text editor and then into Excel, where Excel broke it into columns. The original query was an IL that has strands from two different chains.  So each line of the output is from two different chains.  Columns A, B, C tell about one of the chains, C tells what the chain is, B tells what EC that chain is in, and A tells what Rfam family it is found in, which is the 5.8S rRNA family.  The other chain in the rows is the long LSU chain from RF02543.  It's definitely confusing, but on the other hand, it's powerful!
http://rna.bgsu.edu/correspondence/map_across_species?id=IL_6ZMI_019&scope=EC only gives the alignment within the equivalence class
https://rna.bgsu.edu/correspondence/map_across_species?id=IL_7K00_019&scope=EC&resolution=2.5A is a nice small equivalence class alignment
https://rna.bgsu.edu/correspondence/map_across_species?id=IL_7K00_019&scope=EC&resolution=4.0A is big!
http://rna.bgsu.edu/correspondence/map_across_species?id=IL_6ZMI_019&format=json is JSON format.  There is a field for the query, and then several others.  Hopefully it's easy for you to open this with an interface that shows the structure of a JSON object.
http://rna.bgsu.edu/correspondence/map_across_species?id=5J7L|1|AA|U|70,5J7L|1|AA|A|98&scope=molecule&resolution=2.5A does the same across three Rfam families.  Pretty cool!  Except that many of the alignments are not working out.
...
def map_across_species(nt_lists,scope='Rfam',resolution='3.0A',depth=1,match='full'):
    """

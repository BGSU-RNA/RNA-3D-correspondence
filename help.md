...
Using 5AJ3 as a starting point in the query, R3DMCS maps its 18 nucleotides to other 3D structures which also map to [Rfam family RF00177](https://rfam.org/family/SSU_rRNA_bacteria).  The query has depth=1, so only on structure, the representative structure, from each equivalence class is returned.  Thus an instance from 6GAZ appears in the output page, not the query from 5AJ3. The query takes about 17 seconds to return results on 27 corresponding instances.
